...
const newMin = fromDate => startOfDay(fromDate).valueOf();
// const newMax = (newMin >= to) ? newMin + ONE_DAY : to.valueOf();
// const newMax = fromDate => startOfDay().valueOf();
...
const recentDays = _(
  ...
  // filter(e => e.message.includes('/history?')),
  filter(_(propOr('', 'category'), equals('LcmIotCommons::Workers::UserUsageWorker'))),
  sort(descend(prop('time'))),
  take(5),
  // dateFNS around that minutes
  // map(evolve({
  //   time: _(
  //     // e => {
  //     //   const begining = startOfDay(e).valueOf()
  //     //   const since = e - begining - 10_000
  //     //   const max = e - begining + 10_000
  //     // },
  //     e => format(e, 'hh:mm a'),
  //     // e => new Date(e),)
  //
