import {
  ...
  curry,
  keys,
  startsWith, split, last, when, match, tap, mapObjIndexed, map, uniq, curryN, omit
} from 'ramda'
...
export const msgToCategory = cond([
  ...
  [_(prop('message'), test(/fus\//)), always('TODO:PostOnOpsFrChannel')],
  [_(propOr('', 'category'), test(/BatchAutoStartStopJob/)), always('TODO:PostOnOpsFrChannel')],
  [_(propOr('', 'app'), test(/wqwa/)), always('Wqwa:IgnoreIt')],
  [_(propOr('', 'stackTrace'), includes('salesforce')), always('Salesforce')],
  [_(propOr('', 'path'), includes('support/ticket')), always('Salesforce')],
  ...
  [_(prop('message'), includes('Mqtt')), always('Mqtt')],
  ...
  [allPass([
    _(propOr('', 'category'), test(/ProductLookupController|UpdateEHProductDocumentsWorker/)),
  ...
  [_(prop('message'), test(/v1\/instrumentation\/specification/)), always('503:specification')],
  ...
  [_(prop('message'), includes('RedisClient::ConnectionError')), always('Redis:ConnectionError')],
  [_(allPass([
    ...
    ifElse(
      ...
      always('Redis:Down'))],
  [_(prop('message'), includes('fastmeasuncon')), always('fastmeasuncon')],
  ...
  [_(propOr('', 'proc'), includes('racecar_asset_values')), always('Racecar:Postgres')],
  ...
  [_(prop('message'), includes('Errno::ECONNRESET')), always('ECONNRESET')],
  [_(prop('message'), includes('edge_devices')), always('EdgeDevice')],
  [_(propOr('', 'path'), includes('blog')), always('Blog')],
  [_(prop('message'), test(/oauth\/token/)), x('500:Token')],
  [_(propEq('status', 500)),
    ifElse(
      propEq('app', 'entry'),
      always('500:Entry'),
      always('500'))], // maybe to little selective
...
export const countWith = (fn) => (arr) => {
  ...
  const toKeys = reduce((acc, curr) =>
    assoc(uniqueCharsInLog(curr), { ...curr, length: 0  }, acc), {})
