import {
  ...
  sortBy,
  head,
  fromPairs, uniqWith, countBy, intersection, values, evolve, repeat, __, join, pickAll, toLower, replace,length
} from 'ramda'
...
export const countWith = (fn) => (arr) => {
  ...
  const toKeys = R.reduce((acc, curr) =>
    R.assoc(uniqueChars(curr), { ...curr, length: 0  }, acc), {})
  ...
  arr.forEach(e => {
    ...
    if (counts[key]) {
      ...
      const found = pairs.find(([k, v]) => distance(0.85)(e, v))
      if (found) {
