...
export const msgToCategory = cond([
  ...
  [_(prop('message'), test(/fum\//)), always('postOnOpsFrChannel')],
  [_(propOr('', 'stackTrace'), includes('salesforce')), always('salesforce')],
  [_(propOr('', 'stackTrace'), includes('invoice')), always('payments:invoice')],
  [_(propOr('', 'stackTrace'), includes('chargebee')), always('payments')],
  [_(propOr('', 'userAgent'), includes('ChargeBee')), always('payments')],
  [_(prop('message'), includes('reseller')), always('payments')],
  [_(prop('message'), includes('Subscription response')), always('payments:Subscription')],
  [_(prop('message'), allPass([
    includes('/subscriptions/'),
    includes('400')
  ])), always('payments:Subscription')],
  [_(propOr('', 'path'), includes('/checkout')), always('payments:Subscription')],
  [_(prop('message'), includes('InfluxDB')), always('InfluxDB')],
  [_(prop('message'), includes('kafka')), always('Kafka')],
  [_(prop('message'), includes('bdih')), always('Bdih')], // FWR30
  ...
  [_(prop('message'), includes('db backup')), always('Db backup failed')],
