import {
  ...
  reject,
  filter,
  take, find, groupBy, pluck, divide, converge, length, defaultTo, inc, median, clamp
} from 'ramda'
...
export const msgToCategory = cond([
  ...
  [_(prop('message'), test(/OpcUaCheckJob/)), x('Opc')], // OPC
  ...
  [_(prop('message'), test(/v1\/instrumentations/)), x('instrumentations')], // todo maybe 504:instrumentations
  [_(prop('message'), test(/v1\/subscriptions/)), x('subscriptions')], // todo maybe 504:subscriptions
  ...
  [_(prop('message'), test(/PG::ForeignKeyViolation/)),
    _(prop('message'), split('foreign key constraint '), last, split(' '), head, replace(/'/g, ''), concat('PG::ForeignKeyViolation:'))],
